...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	return ((walkingCaloriesWeightMultiplier*weight + (meanSpeed(action, duration)*math.Pow(2)*kmhInMsec)/height) * walkingSpeedHeightMultiplier * weight) * duration * minInH
}
